...
|---|---|---|
| `gym_classes` | Enseignant → Élève | Liste des classes publiée par le prof, lue par le dropdown élève |
...
### Les classes du prof n'apparaissent pas dans le dropdown élève
C'était le comportement attendu avant la correction — chaque appareil avait son propre `localStorage` isolé. Depuis la mise à jour :
- L'enseignant publie ses classes automatiquement dans Firebase (`gym_classes`) à chaque connexion et à chaque ajout/suppression
- L'élève lit ce nœud en temps réel au chargement de la page de connexion
- **Si les classes n'apparaissent toujours pas** : vérifier que l'enseignant a bien ouvert sa session (l'onglet Firebase doit afficher "✅ Firebase connecté automatiquement"), puis demander à l'élève de recharger la page
